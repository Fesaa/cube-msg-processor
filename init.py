...
command_line_options.add_option(CommandLineOption('MinMsg', regex=r'\b(\d{1,5})\b', default_argument=False, return_type=int,
                                info=f'{Fore.CYAN}MinMsg{Style.RESET_ALL}: Minimal messages send in the channel to appear on TotalMessages graph.'))
command_line_options.add_option(CommandLineOption('MinTime', regex=r'\b(\d{1,2}\.{0,1}\d{0,2})\b', default_argument=False, return_type=float,
